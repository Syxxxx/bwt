...
from .const import API_BASE_URL, API_LOGIN_URL, API_DEVICE_DATA_URL,API_DEVICE_REFRESH_URL, DEFAULT_DEVICE_KEY
...
class BWTClient:
    ...
    async def login(self) -> bool:
        ...
        try:
            ...
            async with session.post(f"{API_BASE_URL}{API_LOGIN_URL}", allow_redirects=False, headers=headers, data=form_data ) as response:
                ...
                if cookies:
                    for cookie in cookies:
                        self.session_cookie = f"PHPSESSID={cookie.value}"
                        self.is_authenticated = True
                        _LOGGER.info(f"Successfully logged in to BWT at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                        return True
                else:
                    _LOGGER.error(f"No session cookie found in response {response.cookies} gg")
                    return False
    ...
    async def fetch_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            urlRefresh = f"{API_BASE_URL}{API_DEVICE_REFRESH_URL}?receiptLineKey={self.device_receipt_line_key}"
            url = f"{API_BASE_URL}{API_DEVICE_DATA_URL}?receiptLineKey={self.device_receipt_line_key}"


            async with session.get(urlRefresh, headers=headers) as response:
                if response.status != 200:
                    ...
                    # Retry the request
                    headers = {"Cookie": self.session_cookie} if self.session_cookie else {}
                    async with session.get(urlRefresh, headers=headers) as retry_response:
            ...
            async with session.post(url, headers=headers) as response:
                if response.status != 200:
                    ...
                    # Retry the request
                    headers = {"Cookie": self.session_cookie} if self.session_cookie else {}
                    async with session.post(url, headers=headers) as retry_response:
                        if retry_response.status != 200:
                            _LOGGER.error(f"Failed to fetch data: {retry_response.status}")
                            return {}
